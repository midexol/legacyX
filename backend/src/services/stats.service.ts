...
async function getOrCreateMarketStat() {
  return prisma.marketStat.upsert({
    where: { id: 1 },
    update: {},
...
// README), this just keeps GET /api/stats from looking static.
export async function tickPrice() {
  ...
  await prisma.marketStat.update({ where: { id: 1 }, data: { priceUsd: next } });
  logger.debug({ priceUsd: next }, "Price tick");
}
